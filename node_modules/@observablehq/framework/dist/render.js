...
async function renderPage(page, options) {
  ...
  const sidebar = page.data?.sidebar !== void 0 ? Boolean(page.data.sidebar) : options.sidebar;
...
${renderFiles(files, resolveFile)}` : ""}${page.data?.sql ? `
${Object.entries(page.data.sql).map(([name, source]) => `registerTable(${JSON.stringify(name)}, FileAttachment(${JSON.stringify(source)}));`).join("\n")}` : ""}
${preview ? `
open({hash: ${JSON.stringify(resolvers.hash)}, eval: (body) => eval(body)});
` : ""}${page.code.map(({ node, id }) => `
${transpileJavaScript(node, { id, resolveImport })}`).join("")}`)}
</script>${sidebar ? html`\n${await renderSidebar(title, pages, root, path, search)}` : ""}${toc.show ? html`\n${renderToc(findHeaders(page), toc.label)}` : ""}
...
async function renderSidebar(title = "Home", pages, root, path, search) {
  return html`<input id="observablehq-sidebar-toggle" type="checkbox" title="Toggle sidebar">
<label id="observablehq-sidebar-backdrop" for="observablehq-sidebar-toggle"></label>
<nav id="observablehq-sidebar">
  <ol>
    <label id="observablehq-sidebar-close" for="observablehq-sidebar-toggle"></label>
    <li class="observablehq-link${normalizePath(path) === "/index" ? " observablehq-link-active" : ""}"><a href="${relativePath(path, "/")}">${title}</a></li>
  ...
  <ol>${pages.map(
    ...
    <details${p.pages.some((p2) => normalizePath(p2.path) === path) ? html` open class="observablehq-section-active"` : p.open ? " open" : ""}>
      ...
      <ol>${p.pages.map((p2) => renderListItem(p2, path))}
      </ol>
...
function renderListItem(page, path) {
  return html`\n    <li class="observablehq-link${normalizePath(page.path) === path ? " observablehq-link-active" : ""}"><a href="${relativePath(path, prettyPath(page.path))}">${page.name}</a></li>`;
}
