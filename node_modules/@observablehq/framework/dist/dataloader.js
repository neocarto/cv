...
class Loader {
   ...
   */
  useStale;
  constructor({ path, sourceRoot, targetPath, useStale }) {
    this.path = path;
   ...
   */
  static find(sourceRoot, targetPath, { useStale = false } = {}) {
    ...
    for (let parent; true; dir = parent) {
      ...
      if (existsSync(join(sourceRoot, dir)))
        return;
      if (existsSync(join(sourceRoot, parent)))
        break;
    }
  ...
  static findExact(sourceRoot, targetPath, { useStale }) {
    for (const [ext, [command, ...args]] of Object.entries(languages)) {
      if (!existsSync(join(sourceRoot, targetPath + ext)))
        continue;
