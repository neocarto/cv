...
async function getResolvers(page, { root, path }) {
  ...
  for (const value of resolutions.values()) {
    for (const i of await resolveNpmImports(root, value)) {
      if (i.type === "local") {
        ...
        const specifier = `npm:${resolveNpmSpecifier(path2)}`;
...
function resolveFilePath(root, path) {
