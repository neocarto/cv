...
class PreviewServer {
  ...
  _socketServer;
  _verbose;
  constructor({ config, server, verbose }) {
    this._config = config;
  ...
  static async start({ verbose = true, hostname, port, open, ...options }) {
    ...
    if (verbose) {
      console.log(`${green(bold("Observable Framework"))} ${faint(`v${"1.2.0"}`)}`);
  ...
  _handleConnection = async (socket, req) => {
...
function handleWatch(socket, req, config) {
  const { root } = config;
  let path = null;
  let hash = null;
  let html = null;
  ...
  async function hello({ path: initialPath, hash: initialHash }) {
    if (markdownWatcher || attachmentWatcher)
...
function getHtml({ html }, { resolveFile }) {
